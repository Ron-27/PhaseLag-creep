...
# ╔═╡ 2da63bd8-5278-11ef-1957-6dbc387dc664
using SparseArrays, Plots, LaTeXStrings, SpecialFunctions, PlutoUI, DifferentialEquations, DataFrames, CSV, StatsPlots, Turing, LinearAlgebra, Random,  FFTW, Measures, DSP, GLM, MessyTimeSeries, StateSpaceModels, LeastSquaresOptim, Peaks, FlexiMaps, ProgressLogging, Smoothers,LsqFit, SingularSpectrumAnalysis, ControlSystemIdentification, Loess, Interpolations, JLD2, DataInterpolations, SavitzkyGolay; gr()

# ╔═╡ 21c65fa5-f063-421f-8ad0-3a9c0ffd7148
include("scripts/PhaseLagFunctions.jl");

# ╔═╡ 969749e3-bef9-42cf-be54-705896c8eeb3
md" # Generating Synthentic Signals"

# ╔═╡ 91f8a71c-16d0-40ee-99ce-5d137b62a3d1
begin
fs = 10.0 # Sampling Frequnecy [Hz]
𝑓 = 1e-3 # Forcing frequency [Hz]
t_end = 1/𝑓 * 10
t = collect(0.0:1/fs:t_end)
T = Int(ceil((1/𝑓)*fs)) # Number of data-points in one period T
ϕ = [π/6, π/9, π/7.5, π/3]
ϕ2 = [π/6 + π/15, π/9 + π/10, π/7.5 + π/5, π/3 + π/3]
a = [0.001, 0.003, 0.004]
a2 = [0.002, 0.005, 0.009]
b = [1e-8, 1e-7 ,1e-6]
b2 = [1e-6, 1e-5 ,1e-5]
end;

# ╔═╡ 5c0c7802-8185-41ad-b26d-79b17a52ebf1
# Building matrices of time-series signals comprehension.
# i (row index) is the phase ϕ[i]
# j (column index) is the trend coefficients a[j] / b[j]
begin
sig_linear = [@. sin(2*π*𝑓*t - ϕ[i]) + a[j]*t for i in 1:length(ϕ), j in 1:length(a)]
sig_nonlinear = [@. sin(2*π*𝑓*t - ϕ[i]) + b[j]*t^2 + a[j]*t for i in 1:length(ϕ), j in 1:length(a)]
sig_linear2 = [@. 5*sin(2*π*𝑓*t - ϕ2[i]) + a2[j]*t for i in 1:length(ϕ2), j in 1:length(a2)]
sig_nonlinear2 = [@. 5*sin(2*π*𝑓*t - ϕ2[i]) + b2[j]*t^2 + a2[j]*t for i in 1:length(ϕ2), j in 1:length(a2)]
end;

# ╔═╡ 2581a4ca-c3e5-4628-a2c2-7a38fd73cbb2
md" # Filtering"

# ╔═╡ d81e4b3c-6a6d-499d-8c30-41013d468534
begin
sig_linear_detrend = [syn_filter(sig_linear[i,j],𝑓,fs) for i in 1:length(ϕ), j in 1:length(a)]
sig_nonlinear_detrend = [syn_filter(sig_nonlinear[i,j],𝑓,fs) for i in 1:length(ϕ), j in 1:length(a)]
sig_linear_detrend2 = [syn_filter(sig_linear2[i,j],𝑓,fs) for i in 1:length(ϕ2), j in 1:length(a2)]
sig_nonlinear_detrend2 = [syn_filter(sig_nonlinear2[i,j],𝑓,fs) for i in 1:length(ϕ2), j in 1:length(a2)]
end;

# ╔═╡ 84c068b5-61d9-41be-aaab-c496caf5f1b8
md" ## Removing transient effect"

# ╔═╡ a6f03ab8-ff71-450a-86fb-f8dcaca08481
begin
N_t = 2 # Number of periods to clear due to transient filter effects
t_filt = t[N_t*T:end]
	
sig_linear_detrend_clear = [sig_linear_detrend[i,j][N_t*T:end] for i in 1:length(ϕ), j in 1:length(a)]
sig_nonlinear_detrend_clear = [sig_nonlinear_detrend[i,j][N_t*T:end] for i in 1:length(ϕ), j in 1:length(a)]

sig_linear_detrend_clear2 = [sig_linear_detrend2[i,j][N_t*T:end] for i in 1:length(ϕ2), j in 1:length(a2)]
sig_nonlinear_detrend_clear2 = [sig_nonlinear_detrend2[i,j][N_t*T:end] for i in 1:length(ϕ2), j in 1:length(a2)]
end;

# ╔═╡ 7cd236d1-f2ea-4955-b47b-eee2a2262ed4
md" ## Extracting phase"

# ╔═╡ 0199f4ea-f32a-472d-bf4a-c787b33f5513
begin
sig_linear_phase = [FFT_phase(sig_linear_detrend_clear[i,j], t_filt, fs) for i in 1:length(ϕ), j in 1:length(a)]
sig_nonlinear_phase = [FFT_phase(sig_nonlinear_detrend_clear[i,j], t_filt, fs) for i in 1:length(ϕ), j in 1:length(a)]

sig_linear_phase2 = [FFT_phase(sig_linear_detrend_clear2[i,j], t_filt, fs) for i in 1:length(ϕ2), j in 1:length(a2)]
sig_nonlinear_phase2 = [FFT_phase(sig_nonlinear_detrend_clear2[i,j], t_filt, fs) for i in 1:length(ϕ2), j in 1:length(a2)]
end;

# ╔═╡ 8016f85e-cc51-4564-8c4a-60201b51411f
md" ## Figure 2"

# ╔═╡ 95afe1e5-46e0-425c-b214-ee35bbfb2201
begin
sp = plot(layout=(1,2))
plot(sp[1],legend=:outerright)
	for i in 1:length(a)
		plot!(sp[1], t, sig_linear[1,i], label=L"h_{%$(i)}", linewidth=3.3)
		plot!(sp[1], t, sig_nonlinear[1,i], linewidth=3.3, linestyle=:dot,
			label=L"g_{%$(i)}", ylabel=L"h_{i} \, \, , \, g_{i}")
	end

plot(sp[2],legend=:outerright)
	for i in 1:length(a)
		plot!(sp[2], t, sig_linear_detrend[1,i], 
			label=L"\tilde{h}_{%$(i)}", legend=false, linewidth=3.5)
		plot!(sp[2], t, sig_nonlinear_detrend[1,i], linestyle=:dot, linewidth=3.5, label=L"\tilde{g}_{%$(i)}", 
			ylabel=L"\tilde{h}_{i} \, \, , \, \tilde{g}_{i}")
	end

plot!(sp, fontfamily="Computer Modern",
	  #title="PIL51",
	  titlefontsize=35,
	  tickfontsize=35,
      legendfontsize=39,
...
annotate!(sp[1], -1200.0, 146.0, text("A", :left, 33, "Helvetica Bold"))
annotate!(sp[2], -1150.0, 1.225, text("B", :left, 33, "Helvetica Bold"))
end

# ╔═╡ 30f924af-0d1f-4699-8559-93c6035b506b
md" # Calculating Errors"

# ╔═╡ 6df33523-7b8a-4c88-a28b-df1e7d9490f8
begin
calibration = ϕ[1] - sig_linear_phase[1,1]
calibration2 = ϕ2[1] - sig_linear_phase2[1,1]
calc = sig_linear_phase[:,1] .+ calibration
calc11 = sig_linear_phase[1,:] .+ calibration
calc2 = sig_nonlinear_phase[:,1] .+ calibration
calc22 = sig_nonlinear_phase[1,:] .+ calibration

err = @. abs( calc - ϕ ) / calc
err11 = @. abs( calc11 - ϕ[1] ) / calc11 + 1e-9 * 100
err2 = @. abs( calc2 - ϕ ) / calc2 
err22 = @. abs( calc22 - ϕ[1] ) / calc22 * 100

rel_lin = abs.(sig_linear_phase[:,1] .-  sig_linear_phase2[:,1])
rel_phi = abs.(ϕ - ϕ2)
rel_lin_nonlin = abs.(sig_linear_phase[:,1] .-  sig_nonlinear_phase2[:,1])
rel_nonlinear = abs.(sig_nonlinear_phase[:,1] .-  sig_nonlinear_phase2[:,1])
rel_err_lin = @. abs( rel_lin - rel_phi ) / rel_lin * 100
rel_err_nonlin = @. abs( rel_nonlinear - rel_phi ) / rel_nonlinear * 100
rel_err_lin_nonlin = @. abs( rel_lin_nonlin - rel_phi ) / rel_lin_nonlin * 100
end;

# ╔═╡ bc44e899-73fe-4be8-8386-3704dde41b4b
md" ## Figure 3"

# ╔═╡ 1ad479bb-17f1-46a0-9c2d-7df169f93305
begin
# Data points to be plotted on the x-axis
x_positions = [1.0, 2.0, 3.0]
x_positions_phi = [1.0, 2.0, 3.0, 4.0]

# Corresponding custom labels for the x-axis
x_labels = [L"h_1, g_1", L"h_2, g_2", L"h_3, g_3"]
x_labels_phi = [L"\phi_{rel} = \frac{\pi}{15}", L"\phi_{rel} = \frac{\pi}{10}", L"\phi_{rel} = \frac{\pi}{5}", L"\phi_{rel} = \frac{\pi}{3}"]


sp2 = plot(layout=(1,2))
scatter!([x_positions x_positions], [err11 err22], labels=[L"h_i" L"g_i"], 
        xticks=(x_positions, x_labels), yscale=:log10, legend=:topleft,
		markersize = 19,markerstrokewidth=0.2, subplot=1)

scatter!([x_positions_phi x_positions_phi], [rel_err_lin rel_err_nonlin rel_err_lin_nonlin], labels=["linear" "nonlinear" "mix"], 
        xticks=(x_positions_phi, x_labels_phi), yscale=:log10, legend=:bottom,  			markersize = 19,markerstrokewidth=0.3, subplot=2,
		ylimits=(10^(-3), 10^(-1.7)),
)

plot!(sp2, fontfamily="Computer Modern",
	  #title="PIL51",
	  titlefontsize=35,
	  tickfontsize=32,
      legendfontsize=35,
...
annotate!(0.75, 1e-2, text("A", :left, 33, "Helvetica Bold"), subplot=1)
annotate!(π/4.9, 10^(-1.69), text("B", :left, 33, "Helvetica Bold"), subplot=2)
end

# ╔═╡ f1d83c77-30b0-4483-be5e-0e994f96ab69
md" # Imports"

# ╔═╡ 35ea2dd0-94e7-4e2a-9473-cd94376e3636
TableOfContents()
...
Smoothers = "012d725f-b4dd-4916-8440-03955fc52575"
...
[compat]
CSV = "~0.10.14"
ControlSystemIdentification = "~2.10.0"
DSP = "~0.7.9"
DataFrames = "~1.6.1"
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "0878431a2186ce9dda237997c6eb05239fb93b49"
...
deps = ["ChainRules", "ComponentArrays", "ControlSystemsBase", "DSP", "DelimitedFiles", "FFTW", "FillArrays", "ForwardDiff", "ForwardDiffChainRules", "InteractiveUtils", "LinearAlgebra", "LowLevelParticleFilters", "MatrixEquations", "MonteCarloMeasurements", "Optim", "Parameters", "QuadGK", "Random", "RecipesBase", "StaticArrays", "Statistics", "StatsBase", "TotalLeastSquares"]
git-tree-sha1 = "5823dd7697d226f3a5c902ab443e4e4194d50ac6"
uuid = "3abffc1c-5106-53b7-b354-a47bfc086282"
version = "2.10.0"
weakdeps = ["LeastSquaresOptim"]

    [deps.ControlSystemIdentification.extensions]
...
git-tree-sha1 = "b12ae85aa3b61386c96263f8cc5b8e7bc06d5326"
...
deps = ["DelimitedFiles", "Distributions", "ForwardDiff", "Lazy", "LinearAlgebra", "LoopVectorization", "PDMats", "Parameters", "Polyester", "Printf", "Random", "RecipesBase", "Requires", "SciMLBase", "SeeToDee", "SimpleNonlinearSolve", "StaticArrays", "Statistics", "StatsAPI", "StatsBase"]
git-tree-sha1 = "801315be2d973828e8d5216e86ddc2ec889755bc"
...
[[deps.SavitzkyGolay]]
deps = ["LinearAlgebra"]
git-tree-sha1 = "edb75da2852e9c7da19a1a860c3adfa6be5978b1"
...
git-tree-sha1 = "8d38d0dfdee02b1c2b77dff40bb54ce47acd57c7"
...
[[deps.Smoothers]]
deps = ["Dierckx"]
git-tree-sha1 = "fab44b0ca72bc5e44606fa37762850e6952b3a3a"
