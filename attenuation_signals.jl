...
# ╔═╡ c3cbb7bc-5270-11ef-3b6d-4dedf74ec148
using SparseArrays, Plots, LaTeXStrings, SpecialFunctions, PlutoUI, DifferentialEquations, DataFrames,Statistics, CSV, StatsPlots, LinearAlgebra, Random,  FFTW, Measures, DSP, GLM, MessyTimeSeries, StateSpaceModels, LeastSquaresOptim, Peaks, FlexiMaps, ProgressLogging,LsqFit, SingularSpectrumAnalysis, Interpolations, JLD2, DataInterpolations, SavitzkyGolay, FastRunningMedian, Optim; gr()

# ╔═╡ 37c6933a-1fde-4645-a2da-be7ef0203d8e
include("scripts/PhaseLagFunctions.jl");

# ╔═╡ 6316545d-92e7-4229-9f05-823471c891b9
md" # Data Preparation"

# ╔═╡ d73dcef6-0b47-4528-9ccc-fa7b27440e58
md" ## Loading Data"

# ╔═╡ eb550684-cb1a-4c41-8d5e-992ec77c5427
md"""
Each experiment is going into a dataframe, to explore the dataframe of the PMMA experiment for example, you can use:
```julia
dataframes["raw_g1"]
```
"""

# ╔═╡ e6760bca-a5f2-4ce5-b22c-e3df1e9352b8
begin
paths = [
    "Nir data/load_25mN/pmma_A10_per100/pmma_A10_per100_Test1.csv",
    "Nir data/load_25mN/pmma_A10_per100/pmma_A10_per100_Test2.csv",
    "Nir data/load_25mN/pmma_A10_per100/pmma_A10_per100_Test3.csv",
	"Nir data/load_50mN/period_100s/hal_A5_per100_Test1.csv",
	"Nir data/load_50mN/period_100s/hal_A5_per100_Test2.csv",
	"Nir data/load_50mN/period_100s/hal_A5_per100_Test3.csv"	
]

# Dictionary to store dataframes
dataframes = Dict{String, DataFrame}()
# Number of digits to round the time measurements
digits = 3

# Create dataframes from the paths, named 'raw_g1, raw_g2, ..'
for (i, path) in enumerate(paths)
    raw = data_read(path)
    raw_g = df_good(raw, digits)
    dataframe_name = "raw_g$i"
    dataframes[dataframe_name] = raw_g
end
end

# ╔═╡ ff227291-cf00-4c6e-959e-6e4d3e2324d5
md" ## Slicing and removing mean"

# ╔═╡ 5b7396e7-7f4b-466f-bd59-99e179cf656e
md" Preparing the PMMA data for filtering:"

# ╔═╡ 47cd81ee-6ca4-4963-b42c-c6df7f3daef1
begin
#New sliced DataFrame with subtracted mean in the load 
df_p1 = dataframes["raw_g1"][10002:97866, :] 
df_p1.load_mN .= df_p1.load_mN.- mean(df_p1.load_mN)
df_p2 = dataframes["raw_g2"][10002:98016, :] 
df_p2.load_mN .= df_p2.load_mN.- mean(df_p2.load_mN)
df_p3 = dataframes["raw_g3"][10002:97985, :] 
df_p3.load_mN .= df_p3.load_mN.- mean(df_p3.load_mN)
end;

# ╔═╡ 6797994a-188b-44a4-846f-92165b48f0ba
md" Preparing the halite data for filtering:"

# ╔═╡ 0166efa0-311b-4665-9591-ab43663c120d
begin
#New sliced DataFrame with subtracted mean in the load 
df_h1 = dataframes["raw_g4"][19502:97878, :] 
df_h1.load_mN .= df_h1.load_mN.- mean(df_h1.load_mN)
df_h2 = dataframes["raw_g5"][19502:97946, :] 
df_h2.load_mN .= df_h2.load_mN.- mean(df_h2.load_mN)
df_h3 = dataframes["raw_g6"][19502:97926, :] 
df_h3.load_mN .= df_h3.load_mN.- mean(df_h3.load_mN)
end;

# ╔═╡ 58210c96-9454-4ee7-9ffc-0a85e4e01c29
md" # Applying the filter"

# ╔═╡ 914f5fc9-3b72-4178-991d-2554a8ba163b
function filter_strain(df)
	f1 = forcing_freq(df.time_s, df.load_mN)
	fs1 = 1/(df.time_s[2] - df.time_s[1])
	ε_f = filter_high(df.eps_nm, f1, fs1)
	return ε_f
end

# ╔═╡ 8706c525-9336-455d-9c4c-54e58203dbd2
begin
#PMMA
df_p1.ε_f = filter_strain(df_p1)
df_p2.ε_f = filter_strain(df_p2)
df_p3.ε_f = filter_strain(df_p3)
#Halite
df_h1.ε_f = filter_strain(df_h1)
df_h2.ε_f = filter_strain(df_h2)
df_h3.ε_f = filter_strain(df_h3)
end;

# ╔═╡ e7d31ceb-edeb-4189-9a13-8532ffb13b6a
md" # Plotting Figure 4"

# ╔═╡ c2fa31a3-a110-4186-81d4-018a08d36faa
begin
df_plots = [df_p1, df_p2, df_p3, df_h1, df_h2, df_h3]
...
a4_width_mm = 178
a4_height_mm = 178
mm_to_pixels = 3.78  # 1mm ≈ 3.78 pixels

plot_width_pixels = a4_width_mm * mm_to_pixels
plot_height_pixels = (a4_height_mm * mm_to_pixels) * 0.7  #Aspect ratio
...
plot!(sp[1], df_plots[3].time_s, df_plots[3].eps_nm, linewidth=1,
		xlabel="Time [sec]", ylabel="Depth [nm]")
...
plot!(sp[2], df_plots[3].time_s, df_plots[3].ε_f, linewidth=0.65, ylimit=(-750,150),
	xlabel="Time [sec]", ylabel="Depth [nm]")
lens!( [400, 800], [-70, 70];
       inset=(2, bbox(0.21, 0.5, 0.7, 0.3)), 
       subplot=5, ticks=false,xlabel="", framestyle=:box,
       lw=1, 
		ls=:dot, 
		lc=:black
		)
...
plot!(sp[4], df_plots[6].time_s, df_plots[6].ε_f, linewidth=0.65, ylimit=(-750,150),
			xlabel="Time [sec]", ylabel="Depth [nm]")
lens!( [400, 800], [-40, 40];
       inset=(4, bbox(0.163, 0.5, 0.7, 0.3)), 
       subplot=6, ticks=false,grid=true, framestyle=:box,
       lw=1, 
		ls=:dot, 
		lc=:black
		)
...
plot!(sp, fontfamily="Computer Modern",
	  ...
	  #minorgrid = true,
      margin=5mm,
      size=(plot_width_pixels, plot_height_pixels),
      dpi=300
      )


#Annotations
annotate!(sp[1], -9, 3410, text("A", :left, tick_font_size, "Helvetica Bold"))
annotate!(sp[2], -50, 200, text("B", :left, tick_font_size, "Helvetica Bold"))
annotate!(sp[3], 70, 3640, text("C", :left, tick_font_size, "Helvetica Bold"))
annotate!(sp[4], 35, 205, text("D", :left, tick_font_size, "Helvetica Bold"))

end

# ╔═╡ a4fa7bf2-ea69-4126-8e0b-18960be452cb
md" # Imports"

# ╔═╡ bcc06e2e-2912-421b-be12-b35d0b723b63
TableOfContents()
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "71b19ef4a5a4e3beb5ed1ce9870e28008101efc5"
